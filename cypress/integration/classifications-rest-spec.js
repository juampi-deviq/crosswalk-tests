context('Testing REST responses for standards', ()=>{
    ...
    it('Validate that requesting for an specific standard version we always get response for the closest one valid',()=> {
        cy.request({
            url: `${Cypress.config().baseUrl}/api/standards/MasterFormat`,
            headers: {'Authorization': key},
            failOnStatusCode: false
        }).then((res) => {
    ...
    it('Searching for classifications based on titles for all standards',()=>{
    ...
    //Server crashes after aprox 1097 sec with assertions. works without assertions
    ...
    it.skip('Using bad formed request url throws an 400 error code',()=> {
        const standard = 'MasterFormat'
        const rest_url = `https://api.crosswalk.digital/v2.1/api/standards/${standard}/classif/any` //its "classifications" not classification (misspending)
        cy.request({
            headers: {'Authorization': key},
            url: rest_url,
            qs: {code: key},
    ...
    it.skip('Querying for non-valid standard throws an 404 error code',()=> {
        ...
        cy.request({
            ...
            console.log(response)
            expect(response.status, "A 404 code (not found) its expected here").to.eq(404)
